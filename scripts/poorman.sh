...
run-gdb()
{
	exec gdb -q -ex "target remote localhost:1234" -ex "set confirm off" \
		-ex "set pagination 0" -ex "handle signal SIGINT stop" \
		$BASE_DIR/linux/vmlinux 1> $POORPIPE 2> $POORPIPE << EOF
define runloop
while (1)
t a a bt
echo Ö
